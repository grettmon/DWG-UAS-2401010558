...
function hapus(id) {
    $.ajax({
        ...
        success: function (dt) {
            $("#infSukses").show();
            $("#infSukses").html("Pembatalan jasa sukses");
            setTimeout(window.location.replace("pesanan jasa.html"), 3000);
        },
        error: function () {
            $("#infError").show();
            $("#infError").html("Pengapusan jasa Gagal");
            setTimeout(window.location.replace("pesanan jasa.html"), 3000);
        },
    });
}
$(function () {
    $.ajax({
        ...
        error: function () {
            console.log("Gagal Membaca data jasa yang diambil");
        },
    });
});
